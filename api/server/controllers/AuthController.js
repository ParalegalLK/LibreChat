...
const refreshController = async (req, res) => {
  ...
  if (token_provider === 'openid' && isEnabled(process.env.OPENID_REUSE_TOKENS)) {
    ...
    try {
      ...
      /** Asgardeo rejects the full auth scope on refresh; only send scope when OPENID_REFRESH_SCOPE is explicitly set */
      if (process.env.OPENID_REFRESH_SCOPE) {
        refreshParams.scope = process.env.OPENID_REFRESH_SCOPE;
      } else {
        delete refreshParams.scope;
      }
      logger.debug('[refreshController] OpenID refresh params', {
        has_scope: Boolean(refreshParams.scope),
