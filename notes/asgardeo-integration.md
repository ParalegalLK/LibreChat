...
This guide covers integrating Asgardeo (WSO2 IDaaS) as the OpenID Connect authentication provider for LibreChat deployed at `https://www.desaram.ai`.

## Overview

- **Protocol:** OpenID Connect (Authorization Code flow with PKCE)
- **Provider:** Asgardeo (`https://api.asgardeo.io/t/desaram`)
...
```
https://www.desaram.ai/oauth/openid/callback
```
...
```env
DOMAIN_CLIENT=https://www.desaram.ai
DOMAIN_SERVER=https://www.desaram.ai
```
...
OPENID_ISSUER=https://api.asgardeo.io/t/desaram/oauth2/token
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Asgardeo button goes to `localhost` | `DOMAIN_SERVER` is set to `localhost` | Set to `https://www.desaram.ai` |
...
- **Discovery URL:** `https://api.asgardeo.io/t/desaram/oauth2/token/.well-known/openid-configuration`
...
```
Browser
  → https://www.desaram.ai (OpenResty, SSL termination)
...
2. User visits `https://www.desaram.ai`
